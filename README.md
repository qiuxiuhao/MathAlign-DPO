...
> **当前状态：Stage 5 — 统一 Mini 评测入口已实现，等待 MPS 运行验收。**  
> Stage 5 可用同一 tokenizer、prompt 和 deterministic generation 参数比较
> Base/SFT/DPO，并输出 exact match、答案提取率、生成长度、耗时和 preference
> diagnostics。RTX 4090 正式实验和消融实验仍为 Planned。
...
要求该 SFT run 不是 smoke run，并且 SFT 训练样本数为 256。DPO 自身样本数由
`dpo.train_samples` 和 `dpo.validation_samples` 控制；当前 Mini 配置在真实
Qwen tokenizer 过滤后使用 179 条 train 和 21 条 validation 偏好数据。
...
长度；超长样本会被过滤并记录，不会交给 Trainer 静默截断，也不会从 formal
DPO 池补样。

Mac MPS Mini DPO 当前关闭训练中间 eval，只在训练完成后显式 evaluate。代码会
先预计算 reference log-prob，并关闭 Transformers 的 NaN/Inf 日志过滤；如果
DPO loss/log-prob 出现 NaN/Inf 或全 0 信号，运行会失败而不会发布 completed
目录。

---

### 7.4 Stage 5 Mini 统一评测命令

Stage 5 必须显式指定已完成的 Stage 3 SFT run 和已完成的 Mini-only Stage 4
DPO run。旧的 formal-pool 扩展 DPO run 会被拒绝。
...
Mini 评测使用 Stage 2 `views.mini.step.evaluation` 的固定 32 条样本；smoke
使用 16 条。Preference accuracy 是诊断指标，不作为正式数学能力结论。
